...
async function updateDashboardCounts() {
    try {
        ...
        const { count: rotasCount } = await supabase
            .from('rotas')
            .select('*', { count: 'exact', head: true });
...
function renderUserTable(users) {
    ...
    tbody.innerHTML = users.map(user => `
        <tr>
            <td>
                <div class="td-name">
                    <div class="td-avatar ${user.role === 'admin' ? 'green' : ''}">
                        ${getInitials(user.full_name || user.email)}
                    </div>
                    <div>
                        <div class="td-main">${user.full_name || 'Sem nome'}</div>
                        <div class="td-sub">ID: ${user.id.slice(0,8)}...</div>
                    </div>
                </div>
            </td>
            <td>${user.email || '-'}</td>
            ...
            <td class="td-actions">
                ...
                <button class="action-btn del" onclick="toggleUserStatus('${user.id}', '${user.status}')" title="${user.status === 'active' ? 'Desativar' : 'Ativar'}">
...
window.editUser = async function(userId) {
    ...
    modalBody.innerHTML = `
        <div class="modal-form-group">
            ...
            <input type="text" class="modal-input" id="editUserName" value="${user.full_name || ''}">
        </div>
        <div class="modal-form-group">
            <label class="modal-label">Email</label>
            <input type="email" class="modal-input" id="editUserEmail" value="${user.email || ''}" disabled>
        </div>
        <div class="modal-form-group">
        ...
        <div class="modal-form-group">
            ...
            <select class="modal-select" id="editUserStatus">
    ...
    try {
        ...
        if (rotaCatFilter) {
            rotaCatFilter.innerHTML = '<option value="">Todas as categorias</option>' + 
                categoriasList.map(cat => `<option value="${cat.id}">${cat.nome}</option>`).join('');
        }
...
window.openModalCategoria = function() {
    ...
    modalBody.innerHTML = `
        <div class="modal-form-group">
            ...
            <input type="text" class="modal-input" id="catNome" placeholder="Ex: Histórico, Natureza...">
        </div>
        <div class="modal-row">
            <div class="modal-form-group">
                <label class="modal-label">Cor (opcional)</label>
                <input type="color" class="modal-input" id="catCor" style="height:42px" value="#979d23">
            </div>
            <div class="modal-form-group">
                <label class="modal-label">Símbolo/Ícone (opcional)</label>
                <input type="text" class="modal-input" id="catSimbolo" placeholder="🏰 🌲 ⛪">
            </div>
        </div>
    `;
...
window.editCategoria = async function(id) {
    ...
    modalBody.innerHTML = `
        <div class="modal-form-group">
            <label class="modal-label">Nome da categoria</label>
            <input type="text" class="modal-input" id="catNome" value="${cat.nome}">
        </div>
        <div class="modal-row">
            <div class="modal-form-group">
                <label class="modal-label">Cor</label>
                <input type="color" class="modal-input" id="catCor" style="height:42px" value="${cat.cor || '#979d23'}">
            </div>
            <div class="modal-form-group">
                <label class="modal-label">Símbolo/Ícone</label>
                <input type="text" class="modal-input" id="catSimbolo" value="${cat.simbolo || ''}">
            </div>
        </div>
    `;
    
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-tag"></i> Editar Categoria';
    document.getElementById('modalOverlay').classList.add('show');
};
...
window.filterPostos = function() {
    ...
    if (catFilter) {
        filtered = filtered.filter(local => 
            local.categorias && local.categorias.some(c => c.categoria_id === catFilter)
        );
    }
...
function renderPostosGrid(locais) {
    ...
    grid.innerHTML = locais.map(local => {
        const categoria = local.categorias && local.categorias.length > 0 
            ? categoriasList.find(c => c.id === local.categorias[0].categoria_id)
            : null;
        
        return `
            <div class="posto-card">
                <div class="posto-icon" style="background: ${categoria?.cor || '#e5e7eb'}20; color: ${categoria?.cor || '#979d23'}">
                    ${categoria?.simbolo || '📍'}
                </div>
                <div class="posto-info">
                    <div class="posto-name">${escapeHtml(local.nome)}</div>
                    <div class="posto-cat">
                        ${categoria ? `<span style="color:${categoria.cor}">${categoria.simbolo || ''} ${categoria.nome}</span>` : 'Sem categoria'}
                        <span>📍 ${local.latitude.toFixed(4)}, ${local.longitude.toFixed(4)}</span>
                    </div>
                    <div class="posto-desc">${local.descricao ? local.descricao.substring(0, 100) + (local.descricao.length > 100 ? '...' : '') : 'Sem descrição'}</div>
                </div>
                <div class="posto-actions">
                    <button class="action-btn edit" onclick="editLocal(${local.id})" title="Editar"><i class="fas fa-edit"></i></button>
                    <button class="action-btn del" onclick="deleteLocal(${local.id})" title="Eliminar"><i class="fas fa-trash"></i></button>
                </div>
            </div>
        `;
    }).join('');
...
window.openModalPosto = function() {
    ...
    modalBody.innerHTML = `
        <div class="modal-form-group">
            ...
            <input type="text" class="modal-input" id="localNome" placeholder="Ex: Castelo de Sesimbra">
        </div>
        <div class="modal-form-group">
            <label class="modal-label">Descrição</label>
            <textarea class="modal-textarea" id="localDescricao" placeholder="Descrição do ponto turístico..."></textarea>
        </div>
        <div class="modal-form-group">
        ...
        <div class="modal-form-group">
            ...
            <div class="map-instruction">
                <i class="fas fa-info-circle"></i> Clique no mapa para definir a localização
            </div>
...
window.editLocal = async function(id) {
    ...
    modalBody.innerHTML = `
        ...
        <div class="modal-form-group">
            <label class="modal-label">Localização</label>
            <div class="map-instruction">
                <i class="fas fa-info-circle"></i> Clique no mapa para ajustar a localização
            </div>
            <div id="locationMapContainer" class="location-map-container">
                <div id="locationPickerMap" class="location-map"></div>
            </div>
            <div class="location-coords-display">
                <div class="coord-item"><i class="fas fa-map-pin"></i> <span id="coordLatDisplay">${local.latitude.toFixed(6)}</span></div>
                <div class="coord-item"><i class="fas fa-map-pin"></i> <span id="coordLngDisplay">${local.longitude.toFixed(6)}</span></div>
            </div>
        </div>
    `;
...
window.filterRotas = function() {
    ...
    if (catFilter) {
        filtered = filtered.filter(rota => 
            rota.categorias && rota.categorias.some(c => c.categoria_id === catFilter)
        );
    }
...
function renderRotasGrid(rotas) {
    ...
    grid.innerHTML = rotas.map(rota => {
        const categoria = rota.categorias && rota.categorias.length > 0 
            ? categoriasList.find(c => c.id === rota.categorias[0].categoria_id)
            : null;
        ...
        return `
            <div class="route-card">
                <div class="route-img" style="background: ${categoria?.cor || '#979d23'}20">
                    <div class="route-img-icon">${categoria?.simbolo || '🗺️'}</div>
                </div>
                <div class="route-body">
                    <div class="route-name">${escapeHtml(rota.nome)}</div>
                    <div class="route-meta">
                        <span><i class="fas fa-map-marker-alt"></i> ${pontosCount} pontos</span>
                        <span><i class="fas fa-layer-group"></i> ${categoria?.nome || 'Sem categoria'}</span>
                    </div>
...
async function saveCategoria() {
    ...
    const data = {
        nome: nome,
        cor: document.getElementById('catCor')?.value || null,
        simbolo: document.getElementById('catSimbolo')?.value || null
    };
...
async function saveLocal() {
    ...
    if (!nome) {
        showToast('Nome do ponto é obrigatório', 'warning');
        return;
    }
    
    if (currentLat === null || currentLng === null) {
        showToast('Selecione a localização no mapa', 'warning');
        return;
    }
    ...
    if (result.error) {
        showToast('Erro ao guardar ponto', 'error');
        return;
    }
